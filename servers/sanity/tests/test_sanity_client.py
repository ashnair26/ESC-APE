...
class TestSanityClient:
    ...
    async def test_from_env(self):
        ...
        with temp_env_vars(
            ...
            with patch("servers.sanity.client.get_secret", AsyncMock(return_value=None)):
                client = await SanityClient.from_env()
    ...
    async def test_from_env_defaults(self):
        ...
        with temp_env_vars(
            SANITY_PROJECT_ID="test-project",
            SANITY_API_TOKEN="test-token"  # Add this to avoid the error
        ):
            # Mock the get_secret function
            with patch("servers.sanity.client.get_secret", AsyncMock(return_value=None)):
                client = await SanityClient.from_env()
    ...
    @pytest.mark.asyncio
    async def test_from_env_missing_vars(self):
        """Test creating a client with missing environment variables."""
        with temp_env_vars():
            # Mock the get_secret function
            with patch("servers.sanity.client.get_secret", AsyncMock(return_value=None)):
                with pytest.raises(ValueError):
                    await SanityClient.from_env()
...
@pytest.mark.asyncio
async def test_get_sanity_client():
    ...
    with temp_env_vars(
        ...
        # Mock the get_secret function
        with patch("servers.sanity.client.get_secret", AsyncMock(return_value=None)):
            client = await get_sanity_client()
...
@pytest.mark.asyncio
async def test_get_sanity_client_missing_vars():
    """Test getting a Sanity client with missing environment variables."""
    with temp_env_vars():
        # Mock the get_secret function
        with patch("servers.sanity.client.get_secret", AsyncMock(return_value=None)):
            with pytest.raises(ValueError):
                await get_sanity_client()
