...
class TestAuthMiddleware:
    ...
    async def test_authenticate_api_token_expired_cache(self):
        ...
        assert result.success is False
        assert result.error == "Invalid API token."
...
async def test_create_api_token():
    ...
    with patch("core.auth.set_secret", AsyncMock(return_value=True)), \
         patch("core.auth.pysecrets.token_urlsafe", return_value="test-token"):
...
async def test_revoke_api_token():
    ...
    with patch("core.auth.delete_secret", AsyncMock(return_value=True)):
